...
@click.option("--region",
              type=str,
              help="Region to deploy lambda. According to role.")
@click.option("--role",
              type=str,
              help="Role to use when deploying lambda function.")
@click.option("--image",
              type=str,
              help="You shoul only provide image name, latest will be used.")
@click.option("-t",
              "--timeout",
              type=int, help="Seconds take to complete lambda function.")
@click.option("--memory",
              type=int,
              help="Memory size to provide to lambda function.")
@click.pass_context
def cli(ctx, region, role, image, timeout, memory):
    ...
    click.echo("Timeout of lambda function is {} seconds".format(ctx.obj.timeout))
    click.echo("Memory size of the lambda function is {} MB".format(ctx.obj.memory))
    click.echo("Image that is being used is {}".format(ctx.obj.image))
...
@click.option("-n",
              "--name",
              type=str,
              help="Name to give to the function. If None train")
@click.pass_context
def train(ctx, name):
    if name is None:
        name = "train"
...
@click.option("-n",
              "--name",
              type=str,
              help="Name to give to the function. If None serve")
@click.option("--run-time",
              type=str,
              help="Name to give to the function. If None serve",
              default="python3.8")
@click.pass_context
def serve(ctx, name, run_time):
    if name is None:
        name = "serve"
    session = boto3.Session()
    lambda_client = session.client('lambda', region_name=ctx.obj.region)
