...
@click.group()
@click.option("-p", "--provider", type=str, help="Select cloud provider to deploy ML product")
@click.option("-s", "--service", type=str, help="Select cloud provider service to deploy ML product")
@click.pass_context
def cli(ctx, provider, service):
    """
    Initialize the project.
    You will be asked to give a name to the application.
    It is important that you understand what provider and service want to work with:
...
def start(ctx):
    ...
    """
    click.echo("Using the following cloud provider: " + click.style(ctx.obj.service_provider, bold=True))
    if ctx.obj.service_name == None:
